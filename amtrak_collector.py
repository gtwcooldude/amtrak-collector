...
================================================
Fetches Amtrak train data every 30 seconds and stores to PostgreSQL.
Designed for easy Railway deployment.
"""
...
def init_database():
    ...
    cursor.execute("""
        ...
        CREATE INDEX IF NOT EXISTS idx_tu_timestamp ON trip_updates(timestamp);
    """)
    ...
    cursor.execute("""
        ...
        CREATE INDEX IF NOT EXISTS idx_kpi_timestamp ON kpi_snapshots(timestamp);
    """)
    ...
    # Hourly patterns for forecasting
    cursor.execute("""
...
def store_trip_updates(updates):
    ...
    sql = """
        ...
        VALUES %s
    """
    values = [(u['trip_id'], u['route_id'], u['delay_seconds'], u['stop_id']) for u in updates]
...
def calculate_and_store_kpis(positions, updates):
    ...
    speeds_mph = [(p['speed'] or 0) * 2.237 for p in positions]  # m/s to mph
    ...
    on_time = sum(1 for d in delays if d < 300) if delays else total_trains  # <5 min = on time
...
def main():
    ...
    ║                                                            ║
    ║  Collecting real-time train data 24/7                      ║
    ║  Building historical database for forecasting              ║
    ╚════════════════════════════════════════════════════════════╝
    """)
    ...
    print(f"✅ KPI calculation interval: {KPI_INTERVAL} seconds")
    print()
    ...
    # Run Flask server (this keeps the service alive)
